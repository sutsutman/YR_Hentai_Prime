...
namespace YR_Hentai_Prime_AnimationBed
{
    public class HediffGiver_RandomAgeCurved_NoAlram : HediffGiver
    {
        ...
        public override void OnIntervalPassed(Pawn pawn, Hediff cause)
        {
            ...
            if (Rand.MTBEventOccurs(this.ageFractionMtbDaysCurve.Evaluate(x), 60000f, 60f) && (this.minPlayerPopulation <= 0 || pawn.Faction != Faction.OfPlayer || PawnsFinder.AllMapsCaravansAndTravelingTransportPods_Alive_FreeColonists_NoCryptosleep.Count() >= this.minPlayerPopulation))
            {
                TryApply(pawn, null);
            }
        }
